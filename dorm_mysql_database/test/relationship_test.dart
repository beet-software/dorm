...
import 'package:dorm_mysql_database/src/relationship.dart';
...
class _Readable<Model, I extends Object>
  ...
  Future<List<Model>> peekAll([
    ...
    final Query query = filter.accept(const Query('SELECT * FROM fake'));
    final RegExpMatch? match = RegExp(r'WHERE (.+) =').firstMatch(query.query);
    final String? key = match?.group(1);
    final Object? value = query.params['value'];
    return models.values
        .where((model) => key == null || json(model)[key] == value)
        .toList();
  }
...
void main() {
  ...
  final Relationship relationship = const Relationship();

  test('resolves one-to-one relationships', () async {
    final OneToOneAssociation<_Parent, String, _Child, Query> association =
