...
Dart classes and getters, then regenerate the <i>*.dorm.dart</i> and <i>*.g.dart</i>
parts.

!!! note
    An annotation declares names, fields, identities, or relationships. It
    does not open a connection or perform a database operation. Think of it as
    an instruction for the generator: it describes what a value means, while
    the generated entity and the selected engine use the resulting metadata at runtime.
