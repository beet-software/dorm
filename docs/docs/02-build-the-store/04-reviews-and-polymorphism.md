# Add reviews with embedded and polymorphic content
...
The model declares three content variants:
...
`@PolymorphicField` stores the selected variant in `content` and stores its discriminator in the `type` pivot field. Generation produces `ReviewContentType`, `ReviewContent`, and one concrete class for each annotated variant.
